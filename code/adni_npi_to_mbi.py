...
adni_npi_p = Path("/home/neuromod/wrangling-phenotype/data/adni/NPI_22Aug2023.csv")
qc_pheno_p = Path("/home/neuromod/wrangling-phenotype/outputs/passed_qc_master.tsv")
output_p = Path("/home/neuromod/wrangling-phenotype/mbi.tsv")

# load data
adnimerge_df = pd.read_csv(adnimerge_p, low_memory=False)
npi_df = pd.read_csv(adni_npi_p)
...
# Replace some rougue dates first
...
qc_df_filtered["ses"] = pd.to_datetime(qc_df_filtered["ses"])  # format="%m/%Y"
...
merged_df = pd.merge_asof(
    ...
    by="RID",
    on="ses",
    direction="nearest",  # Finds the closest match, whether before or after
    tolerance=pd.Timedelta(days=183),  # Approximately 6 months
)

# merged_df.to_csv(output_p, sep="\t", index=False)
mbi_df.to_csv(output_p, sep="\t", index=False)
